...
conf.dataDir = fullfile(aba_root, 'registration/groupwise/data/');
conf.numTrain = 15;
conf.numTest = 15;
conf.numClasses = 2;
conf.numWords = 1000;
...
[rig.im1_to_mean.im, rig.im1_to_mean.tf] = aba_rigid_align_new(im1, mean_gray);
[rig.im2_to_mean.im, rig.im2_to_mean.tf] = aba_rigid_align_new(im2, mean_gray);
[rig.im1_to_im2.im, rig.im1_to_im2.tf]  = aba_rigid_align_new(im1, im2);
[rig.im2_to_im1.im, rig.im2_to_im1.tf]  = aba_rigid_align_new(im2, im1);

% Binary registration
[brig.im1_to_mean.im, brig.im1_to_mean.tf] = aba_rigid_align_new(bim1, mean_bin);
[brig.im2_to_mean.im, brig.im2_to_mean.tf] = aba_rigid_align_new(bim2, mean_bin);
[brig.im1_to_im2.im, brig.im1_to_im2.tf]   = aba_rigid_align_new(bim1, bim2);
[brig.im2_to_im1.im, brig.im2_to_im1.tf]   = aba_rigid_align_new(bim2, bim1);
...
rig.im1_to_im2.imwb  = rig.im1_to_im2.im;
rig.im2_to_im1.imwb  = rig.im2_to_im1.im;
rig.im1_to_mean.imwb(rig.im1_to_mean.imwb == 0) = 255; 
rig.im2_to_mean.imwb(rig.im2_to_mean.imwb == 0) = 255; 
rig.im1_to_im2.imwb(rig.im1_to_im2.imwb == 0) = 255;  
rig.im2_to_im1.imwb(rig.im2_to_im1.imwb == 0) = 255;
rig.im1_to_mean.imwb = aba_imcrop_edge(rig.im1_to_mean.imwb);
rig.im2_to_mean.imwb = aba_imcrop_edge(rig.im2_to_mean.imwb);
rig.im1_to_im2.imwb  = aba_imcrop_edge(rig.im1_to_im2.imwb);
rig.im2_to_im1.imwb  = aba_imcrop_edge(rig.im2_to_im1.imwb);
...
% -------------------------------------------------------------------------
%% Groupwise non-linear registration (using MIRT)
% -------------------------------------------------------------------------
% imshow(imclearborder(~im2bw(rig_im2_to_im1)))
% Mwarp = ~imwarp(true(size(im2)), tform);
% rig_im2_to_im1(Mwarp &~imclearborder(Mwarp)) = 255;
% imshow(aba_imcrop_morph(rig_im2_to_im1))
% Irot = imrotate(I,theta);
% Mrot = ~imrotate(true(size(I)),theta);
% Irot(Mrot&~imclearborder(Mrot)) = 255;

imstack = im2double(cat(3, rig.im1_to_mean.im, rig.im2_to_mean.im));
imstack = imstack(:,1:end-1, :);

a = cat(3, imresize(imadjust(rig.im1_to_im2.imwb), [480 480]), imadjust(im2));
a = im2double(a);   
a(a==255) = nan;
...
b_g = mirt2Dgroup_transform(imstack, res2);
%%
...
imshow(b_g(:, :, 1));
subplot(3, 3, 8)
imshow(mean_image(b_g(:, :, 1), b_g(:, :, 2)))
subplot(3, 3, 9)
imshow(b_g(:, :, 2))
% -------------------------------------------------------------------------
%% MIRT on images rigid aligned to mean image
% -------------------------------------------------------------------------
